...
function createFailedDataIngestionLogger(configuration: LoggingConfiguration, logName: string): Logger {
    if (configuration.getDataIngestionLogging()) {
        if (configuration.getDataIngestionDeadLetterPath() == '') {
            ...
            return pino({
                ...
                pipeline: [{
                    ...
                    // without any change.
                    target: 'pino/file',
                    options: { destination: `${configuration.getDataIngestionDeadLetterPath()}/${logName}`, mkdir: true, append: true }
                }]
                }
            });
        }
...
export { createFailedDataIngestionLogger };